...
def dyn(x, c, gamma):
    ...
    """
    weights = [0.5, 0.8, 0.8, 3., 3.]
    dx = np.zeros(len(x))
    for i in range(len(c)):
#        dx += (-2./gamma) * (x - c[i]) * rbf(x, c[i], gamma) + ipt*x
        dx += weights[i] * (-4./gamma) * (x - c[i]) * rbf(x, c[i], gamma)# + ipt*x
    return dx
...
feat_match = 0.5*np.exp(-np.array(all_sents))
...
#                          + noise[t-1, ])
            xhist[t, ] = (xhist[t-1, ] + tau
                          * dyn(xhist[t-1, ], centers, gamma)#, ipt)
                          + noise[t-1, ])
            if sent < 10:#3:
                if t == isi:
...
#                        ipt = feat_match[sent, ].copy()
#                        xhist[t, ] += ipt
#                    elif t >= 2*isi and speed(xhist[t-1, ], centers,
#                                              gamma, feat_match) < 1.:
#                        # Test to see which parse you're in
#                        ipt = [0.0]*ndim
#                        last = np.round(xhist[t-1, ].copy())
